...
async function chatAnswer(message) {
	...
	if (ex > 1) rl += 1;
	const t = rl * 1000;
	await new Promise((r, j) => setTimeout(r, t)); // https://api-sv2.simsimi.net/v2/?text=hello&lc=en&cf=false
	const u = message.slice(0, 1000);
	return axios.get(`https://api-sv2.simsimi.net/v2/`, {
		params: {
			text: u,
			lc: "en"
		}
	}).then(r => r.data.success.replace(/I don't know what you're saying\. Please teach me\.|Sim doesn't know what you are talking about. Please teach me/, "Sorry but i don't speak gibberish").replace(/kemon acho babu/, U[Math.floor(Math.random() * U.length)])).catch(console.error)
		.finally(() => ex--);
}

chatAnswer(process.argv.slice(2).join(" ")).then(console.log);
